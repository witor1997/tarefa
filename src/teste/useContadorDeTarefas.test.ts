...
describe('useContadorDeTarefas', () => {
  it('deve retornar o número correto de tarefas concluídas', () => {
    const { result } = renderHook((props: { tarefas: Tarefa[] }) => useContadorDeTarefas(props.tarefas), {
  ...
  it('deve atualizar o contador quando as tarefas mudarem', () => {
    ...
    const { result, rerender } = renderHook((props: { tarefas: Tarefa[] }) => useContadorDeTarefas(props.tarefas), {
      initialProps: { tarefas: tarefasMock },
    });
